...
import zipfile
import re
from engine_ai import elabora_documenti_banana, genera_archivio_zip, esporta_per_banana, esporta_per_excel_colorato
...
/* ── TASTO SVUOTA FATTURE ── */
div.stButton > button[key="svuota_fatture_btn"] {
    background-color: #1c1c1e !important;
...
div.stButton > button[key="svuota_fatture_btn"]:hover {
...
# ==============================================================================
    ...
    with st.sidebar:
        ...
        try:
            api_key_default = st.secrets["OPENAI_API_KEY"]
        except:
            api_key_default = ""
    ...
    with col_svuota:
        ...
        if st.button("✕ Svuota", key="svuota_fatture_btn"):
    ...
    file_caricati = st.file_uploader(
        ...
        key=f"up_fatture_{st.session_state['uploader_key']}",
    )
    ...
    if file_caricati and api_key_input:
        ...
        if st.button("AVVIA ELABORAZIONE DOCUMENTI", key="elabora_btn_main"):
            with st.spinner("Analisi fiduciaria in corso — elaborazione visiva OCR..."):
                ...
                buf_xl_bytes  = esporta_per_excel_colorato(
                    df_creato,
                    indici_rosso,
                    indici_verdi,
                    indici_errore,
