...
Migliorie v5:
  ...
  - Riga cambio: descrizione in col3, conto Differenza di cambio in dare, stesso ctAvere in avere, importo vuoto
...
6570 Leasing Hardware | 6571 Leasing Software | 6572 Noleggio terminale pagamento e noleggio Hardware | 6573 Noleggio Software
...
INVESTIMENTI (I) = beni durevoli pluriennali destinati all'uso interno:
...
REGOLA M vs I:
  M = consumabile/rivendibile: materie prime, alimenti, bevande, servizi ricorrenti, affitto,
      telefonia, energia, abbonamenti SaaS, materiale consumo, pubblicità, pulizie, CO2/gas bombole
  I = bene durevole uso interno: macchinari, veicoli, arredamento, computer, impianti, software perpetuo

IVA VUOTA (codice_iva = ""):
  - IVA estera:
    IT 22%, DE 19%, FR 20%, UK 20% ecc. -> SEMPRE vuoto
  ...
  - Esente/Fuori campo/0%: "Esente", "Befreit", "Exonéré", "0%", "E", "exempt" -> vuoto
  - Operazione non soggetta (es. art.7 DPR IT) -> vuoto
  - Metodo saldo -> SEMPRE vuoto
  - Nessuna IVA visibile in fattura -> lascia vuoto
  
DOPPIA ALIQUOTA: se ultima pagina mostra righe separate per aliquota,
  genera UNA RIGA PER ALIQUOTA con tipo_riga="iva_split".
  importo_nominale = imponibile + IVA di QUELLA aliquota. La SOMMA deve = totale fattura.
...
def _aggiungi_riga_errore(lista, indici_errore, indici_separatori, contatore, nome_file, messaggio):
    """Aggiunge riga errore (rossa) + riga separatrice azzurra."""
    ...
    riga_err = {
        ...
        "_flag_separatore": False,
    }
    lista.append(riga_err)
    indici_errore.append(contatore)
    # Riga separatrice azzurra sempre dopo l'errore
    riga_sep = {col: None for col in riga_err.keys()}
    riga_sep["Data"]             = ""
    riga_sep["_flag_separatore"] = True
    lista.append(riga_sep)
    indici_separatori.append(contatore + 1)
...
PROCEDURA DI RICERCA IN ORDINE PRIORITARIO:

PASSO 1 — LOGO E INTESTAZIONE (PRIMA pagina, zona superiore):
  Il logo è quasi sempre del fornitore. Leggi il testo DENTRO o SOTTO il logo.
  Cerca il nome aziendale nella zona superiore (header): prima riga, intestazione, ragione sociale.
  Esempi: "WORLDLINE", "RISTORFOODS", "bluwater", "Publineon", "Warriors Weekend",
           "AUXILIA Family Office", "About Food", "Chicco d'Oro", "Marina SA",
           "Sunrise Sagl", "Swisscom", "AIL SA", "HSH Delivery"

PASSO 2 — RIGA "BENEFICIARIO / PAGABILE A / A FAVORE DI":
  Nella sezione di pagamento (QR-bill, bollettino): il conto IBAN è intestato al FORNITORE.
  "Conto / Pagabile a" = il fornitore riceve il pagamento.
  "Pagabile da" = il ricevente paga. NON è il fornitore.

PASSO 3 — IN CALCE AL DOCUMENTO:
  Alcune fatture hanno ragione sociale + indirizzo in fondo alla pagina (es. publineon sagl / strada regina 120).
  Questa è spesso la firma del fornitore.

REGOLE CRITICHE:
  ✗ MAI usare come fornitore il nome del RICEVENTE ({nome_ricevente if nome_ricevente else "l'azienda cliente"})
  ✗ MAI confondere "destinatario" con "fornitore": destinatario = chi riceve la merce = ricevente
  ✗ Se la fattura ha "Intestazione documento" separata da "Spett.le": Intestazione = fornitore, Spett.le = ricevente
  ✓ Includi SEMPRE la forma giuridica se visibile (Sagl/SA/AG/GmbH/Srl/SpA/Inc/Ltd)
  ✓ Se il fornitore è illeggibile: "Fornitore Sconosciuto" con fornitore_sconosciuto:true
▌REGOLA 4 — CONTO DARE E CONTO AVERE (PARTITA DOPPIA)
ACQUISTI (flusso costi):
  ...
  PRIORITA' 2 — ANALISI CONTENUTO FATTURA IN 4 PASSI:
    PASSO 1: Leggi attentamente TUTTE le righe di descrizione prodotti/servizi.
             NON fermarti al nome del fornitore: lo stesso fornitore puo' vendere
             diversi servizi/prodotti nella stessa fattura.
             Cerca parole chiave nelle descrizioni: "gas", "elettricita'", "acqua",
             "manutenzione", "riparazione", "fornitura", "pubblicità", "canone", "abbonamento", ecc.
             Esempio: Stesso fornitore, tre scenari diversi:
             a) "Manutenzione caldaia", "revisione impianto", "assistenza tecnica",
             "riparazione", "Wartung", "Reparatur", "entretien" -> 6050 o 6100 o 6200
             (NON 4000, NON conto investimento)
             Se la fattura ha piu' voci di tipo diverso MA un solo totale, usa il conto
             della voce piu' rilevante per importo.
             b) "Fornitura materiale", "ricambi", "pezzi di ricambio" di importo < CHF 1000
             -> conto costo appropriato (6100, 6200, ecc.)
             c) "Installazione nuovo impianto", "fornitura e posa", bene durevole > CHF 1000
             -> conto investimento appropriato (15xx, 16xx, 17xx)
...
NOTE DI CREDITO / STORNI / IMPORTI NEGATIVI:
  ...
  Parole chiave nota credito:
   ...
   ▌REGOLA 4b — ASSICURAZIONI SOCIALI E IMPOSTA ALLA FONTE (PRIORITÀ ASSOLUTA SUL CONTO DARE)
Queste voci hanno conti FISSI che sovrascrivono qualsiasi altra logica settoriale.
Leggi ATTENTAMENTE la descrizione della fattura per distinguerle.
 
PREMI ASSICURATIVI OBBLIGATORI (costi del personale — classe 5):
  5730 → Assicurazione infortuni LAINF / SUVA / LAA
         Parole chiave: "LAINF", "LAA", "SUVA", "infortuni", "AINP" , "Unfallversicherung",
         "assurance-accidents", "premio infortuni", "Prämie UVG"
  5740 → Assicurazione indennità giornaliera malattia (IJM / KTG)
         Parole chiave: "indennità giornaliera", "IJM", "KTG", "APGM" , "Krankentaggeld",
         "indemnités journalières", "perdita di guadagno malattia",
         "Lohnausfallversicherung", "assurance perte de gain maladie"
  5720 → Previdenza professionale LPP / BVG / secondo pilastro
         Parole chiave: "LPP", "BVG", "secondo pilastro", "2° pilastro",
         "previdenza professionale", "cassa pensioni", "Pensionskasse",
         "prévoyance professionnelle"
  5700 → AVS / AI / IPG / AD (primo pilastro)
         Parole chiave: "AVS", "AI", "IPG", "AD", "AHV", "IV", "EO", "ALV",
         "contributi sociali", "Sozialversicherung"
 
  ⚠ NON confondere il PREMIO (costo aziendale → classe 5) con il RIMBORSO:
    Se l'assicurazione PAGA l'azienda (rimborso infortuni, rimborso malattia):
    -> è una diminuzione di costo -> CtAvere = 5900, CtDare = 1020
    Se l'azienda PAGA il premio all'assicurazione:
    -> è un COSTO -> usa i conti 57xx sopra indicati
 
  ⚠ NON usare mai i conti delle assicurazioni aziendali (6300–6320) per i premi
    delle assicurazioni sociali obbligatorie del personale (LAINF, IGM, LPP, AVS).
    6310 (RC azienda), 6300 (danni), 6311 (garanzie) sono per beni/attività aziendali,
    NON per il personale.
 
IMPOSTA ALLA FONTE:
  5790 → Imposta alla fonte (ritenuta fiscale su salari di dipendenti stranieri)
         Parole chiave: "imposta alla fonte", "ufficio delle imposte alla fonte e del bollo" , "Quellensteuer", "impôt à la source",
         "ritenuta alla fonte".
  ⚠ NON usare 6361 (tasse) per l'imposta alla fonte: è un costo del personale, non una tassa aziendale. 
...
▌REGOLA 5 — IMPORTO LORDO (IVA INCLUSA) — REGOLA CRITICA
...
1. Cerca PRIMA nel documento uno di questi segnali di valuta NON-CHF:
   - Simbolo: €, $, £, ¥
   - Codice: EUR, USD, GBP, JPY, DKK, SEK, NOK, CZK
   - Parola: "euro", "euros", "dollaro", "pound"
   - Paese/Banca emittente: fattura da azienda ITALIANA → quasi certamente EUR
     fattura da azienda AMERICANA → quasi certamente USD
   - IBAN estero: IT*, DE*, FR*, GB* → possibile valuta estera

2. Se trovato segnale NON-CHF: valuta = quella valuta (es. "EUR"), tasso ≠ 1.0
3. Se nessun segnale: valuta = "CHF", tasso = 1.000000

CASI CONCRETI:
  Fattura da azienda italiana con "€ 790,00" o "EUR 790" → valuta=EUR
  Fattura da azienda svizzera con "CHF 150.00" → valuta=CHF
  Fattura con IBAN CH ma importo in EUR → valuta=EUR
  Fattura senza indicazione di valuta → default CHF

TASSO DI CAMBIO:
  CHF: sempre 1.000000
  Altra valuta: inserisci tasso ufficiale AFC del mese della fattura se lo conosci con certezza.
  Se incerto: 0.000000 (l'operatore compilerà manualmente) + nota_cambio="DA COMPILARE"
...
▌REGOLA 7 — RIGA DIFFERENZA CAMBIO (SOLO SE valuta != CHF)
  ...
  Questa riga ha:
    data_fattura: stessa data_fattura della riga principale
    numero_fattura: "" (vuoto)
    fornitore_cliente: "Differenza di cambio - fattura n. [numero_fattura_riga_principale]"
...
▌REGOLA 8 — IVA
  ...
  -> é OBBLIGATORIO generare una riga distinta per ogni aliquota con tipo_riga="iva_split"
  -> Ogni riga iva_split deve condividere data, numero fattura e fornitore
  -> I conti (CtDare) possono differire se la fattura riguarda beni/servizi diversi.
  -> importo_nominale = importo IVA INCLUSA (Imponibile di quella specifica aliquota + IVA di quella specifica aliquota) relativo a QUELLA SPECIFICA ALIQUOTA
  ...
  DIVIETI:  se presente piu di una aliquota NON usare l'imponibile da solo, NON usare il totale fattura generico come importo per una singola riga.
  ...
  SE il ricalcolo non risolve la discrepanza: Imposta il flag flag_errore_iva = true nel JSON di output per permettere la marcatura in GIALLO nel file Excel.
...
"""
    ...
    async def _run_parallel():
        ...
        tasks = [
            _elabora_singola_fattura(client_async, nome, data, prompt_sistema, conto_fornitore_default, sem)
    ...
    # ── Assembla DataFrame ────────────────────────────────────────────────
    #
    # STRUTTURA RIGHE PER FATTURA — ORDINE GARANTITO:
    #
    #   CASO NORMALE (CHF, nessuna eccezione):
    #     [riga registrazione]       → non colorata
    #     [riga separatrice azzurra] → colorata di azzurro
    #
    #   CASO VALUTA ESTERA (EUR/USD/...):
    #     [riga registrazione]       → VERDE
    #     [riga cambio 6949]         → VERDE
    #     [riga separatrice azzurra] → colorata di azzurro
    #
    #   CASO IVA SPLIT:
    #     [riga iva_split aliquota1] → non colorata (verde se valuta estera)
    #     [riga iva_split aliquota2] → non colorata (verde se valuta estera)
    #     [riga cambio 6949]         → VERDE (se valuta estera)
    #     [riga separatrice azzurra] → colorata di azzurro
    #
    #   CASO ERRORE:
    #     [riga errore]              → ROSSA
    #     [riga separatrice azzurra] → vuota
    #
    #   CASO CONTO INCERTO (fornitore_sconosciuto):
    #     [riga registrazione]       → non colorata, cella CtDare GIALLA
    #     [riga separatrice azzurra]
    #
    lista_righe_finali = []
    indici_rosso       = []   # tasso da verificare (rosa)
    indici_verde       = []   # gruppo valuta estera (principale + cambio) → verde
    indici_errore      = []   # errori elaborazione → rosso scuro
    indici_giallo_dare = []   # conto dare incerto → solo cella CtDare gialla
    indici_separatori  = []   # righe separatrici azzurre
    ...
    for risultato in risultati:
        ...
        # ── CASO ERRORE ──────────────────────────────────────────────────
        if risultato["errore"]:
            _aggiungi_riga_errore(lista_righe_finali, indici_errore, indici_separatori,
                                   contatore_righe, nome_originale, risultato["errore"])
            contatore_righe += 2
        ...
        # Determina se la fattura è in valuta estera PRIMA di costruire le righe:
        # tutte le righe del gruppo (principale/iva_split/cambio) diventano verdi.
        valuta_estera = any(
            str(r.get("valuta", "CHF")).upper().strip() not in ("CHF", "")
            for r in registrazioni
            if r.get("tipo_riga") != "cambio"
        )

        righe_fattura         = []
        indici_locali_verde   = []
        indici_locali_rosso   = []
        indici_locali_giallo  = []
        ...
        for i_reg, reg in enumerate(registrazioni):
            ...
            if tipo_riga == "cambio":
                ...
                try:
                    importo_nom = round(float(reg.get("importo_nominale") or 0), 2)
                except (ValueError, TypeError):
                    importo_nom = 0.0
                if importo_nom < 0:
            ...
            if data_fat and data_fat not in ("", "INSERIRE MANUALMENTE"):
                ultima_data_fattura = data_fat

            if tipo_riga == "cambio":
                descrizione_finale = f"Differenza di cambio - fattura n. {num_fattura_principale}"
            elif descrizione:
            ...
            idx_locale = len(righe_fattura) - 1

            # Assegna colori locali
            if valuta_estera:
                # Tutta la fattura in valuta estera → verde
                indici_locali_verde.append(idx_locale)
            elif nota_cambio in ("TASSO DA VERIFICARE", "DA COMPILARE") and tipo_riga != "cambio":
                # Solo il tasso mancante su fattura CHF → rosa
                indici_locali_rosso.append(idx_locale)

            if flag_sconosciuto:
                indici_locali_giallo.append(idx_locale)
                logger.warning("[%s] Fornitore sconosciuto — CtDare=4000 evidenziato", nome_originale)

        # ── Riga separatrice azzurra (sempre presente dopo ogni fattura) ──
        riga_sep = None
        if righe_fattura:
            riga_sep = {col: None for col in righe_fattura[0].keys()}
            riga_sep["Data"]             = ""
            riga_sep["Descrizione"]      = f"pagamento fattura - {num_fattura_principale}"
            riga_sep["CtDare"]           = ct_avere_principale
            riga_sep["CtAvere"]          = "1020"
            riga_sep["_flag_giallo_dare"] = False
            riga_sep["_flag_separatore"]  = True

        # ── Registra righe nel DataFrame globale con indici corretti ──────
        for idx_locale, riga in enumerate(righe_fattura):
            lista_righe_finali.append(riga)
            idx_globale = contatore_righe
            if idx_locale in indici_locali_verde:
                indici_verde.append(idx_globale)
            if idx_locale in indici_locali_rosso:
                indici_rosso.append(idx_globale)
            if idx_locale in indici_locali_giallo:
                indici_giallo_dare.append(idx_globale)
            contatore_righe += 1

        if riga_sep:
            lista_righe_finali.append(riga_sep)
            indici_separatori.append(contatore_righe)
            contatore_righe += 1
    ...
    if not lista_righe_finali:
        df_vuoto = pd.DataFrame(columns=[
            ...
            "Imp. moneta", "Moneta", "Cambio", "Importo CHF", "Cod. IVA",
            "_flag_giallo_dare", "_flag_separatore"
        ])
        return df_vuoto, indici_rosso, indici_verde, indici_errore, indici_giallo_dare, indici_separatori, dizionario_pdf_rinominati
...
# ==============================================================================
# EXPORT PER BANANA CONTABILITÀ+
# ==============================================================================
_COLONNE_IMPORTO = ["Imp. moneta", "Importo CHF"]
_COLONNE_TASSO   = ["Cambio"]

_COLONNE_LARGHEZZE = {
    "Data": 12, "Fattura": 22, "Descrizione": 45,
    "CtDare": 10, "CtAvere": 10,
    "Imp. moneta": 14, "Moneta": 8, "Cambio": 12,
    "Importo CHF": 14, "Cod. IVA": 10,
}


def _prepara_df_base(df: pd.DataFrame) -> pd.DataFrame:
    """Rimuove colonne interne e ordina cronologicamente."""
    df_out = df.drop(columns=["_flag_giallo_dare", "_flag_separatore"], errors="ignore").copy()
...
def esporta_per_banana(df: pd.DataFrame) -> bytes:
    """
    Export Banana Contabilità+.
    Numeri come STRINGHE con PUNTO decimale (es. "1992.45").
    Formato cella "@" per evitare conversioni automatiche di Excel.
    """
    df_out = _prepara_df_base(df)

    def _fmt_punto(val, dec):
        if val is None:
            return None
        try:
            f = float(val)
    ...
    for col in _COLONNE_IMPORTO:
        if col in df_out.columns:
            df_out[col] = df_out[col].apply(lambda v: _fmt_punto(v, 2))
    for col in _COLONNE_TASSO:
        if col in df_out.columns:
            df_out[col] = df_out[col].apply(lambda v: _fmt_punto(v, 6))

    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        df_out.to_excel(writer, index=False, sheet_name="Registrazioni")
...
# ==============================================================================
# EXPORT PER EXCEL CON COLORI
# ==============================================================================

def esporta_per_excel_colorato(
    df: pd.DataFrame,
    indici_rosso: list,
    indici_verde: list,
    indici_errore: list,
    indici_giallo_dare: list,
    indici_separatori: list,
) -> bytes:
    """
    Export Excel con colori identici all'interfaccia Streamlit.
    Numeri come NUMERI REALI con formato VIRGOLA italiana (es. 1992,45).

    Legenda colori:
      VERDE       → riga intera: fattura in valuta estera
                    (riga registrazione + riga cambio 6949)
      ROSA        → riga intera: tasso cambio non disponibile
      ROSSO SCURO → riga intera: errore elaborazione PDF
      GIALLO      → solo cella CtDare: conto incerto (fallback 4000)
      AZZURRO     → riga intera: separatore tra fatture (solo Data)
    """
    from openpyxl.styles import PatternFill, Font, Alignment
    from openpyxl.utils import get_column_letter

    # ── Palette colori ────────────────────────────────────────────────────
    FILL_VERDE   = PatternFill("solid", fgColor="FFB7E1CD")
    FILL_ROSSO   = PatternFill("solid", fgColor="FFFF9999")
    FILL_ERRORE  = PatternFill("solid", fgColor="FFFFC7CE")
    FILL_GIALLO  = PatternFill("solid", fgColor="FFFFFF00")
    FILL_AZZURRO = PatternFill("solid", fgColor="FFD6E4F0")
    FILL_HEADER  = PatternFill("solid", fgColor="FF2F5496")
    

    FONT_HEADER  = Font(bold=True, color="FFFFFFFF", size=10)
    FONT_ERRORE  = Font(bold=True, color="FF9C0006", size=10)
    FONT_GIALLO  = Font(bold=True, color="FF7F6000", size=10)
    FONT_VERDE   = Font(color="FF1D6132", size=10)
    FONT_NORMALE = Font(size=10)

    # Formato numeri italiani: virgola decimale
    FMT_IMPORTO = '#.##0,00'
    FMT_TASSO   = '#.##0,000000'

    df_out = _prepara_df_base(df)

    # Converti in float (numeri reali, non stringhe)
    def _to_float(val, dec):
        if val is None:
            return None
        try:
            f = float(val)
    ...
    for col in _COLONNE_IMPORTO:
        if col in df_out.columns:
            df_out[col] = df_out[col].apply(lambda v: str(_to_float(v, 2)).replace('.', ',') if pd.notna(v) else v)
    for col in _COLONNE_TASSO:
        if col in df_out.columns:
            df_out[col] = df_out[col].apply(lambda v: str(_to_float(v, 6)).replace('.', ',') if pd.notna(v) else v)

    # Set per lookup O(1)
    set_verde      = set(indici_verde)
    set_rosso      = set(indici_rosso)
    set_errore     = set(indici_errore)
    set_giallo     = set(indici_giallo_dare)
    set_separatori = set(indici_separatori)
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        for i, col_name in enumerate(df_out.columns, 1):
            ws.column_dimensions[get_column_letter(i)].width = _COLONNE_LARGHEZZE.get(col_name, 14)
        ...
        # Indici colonne utili (1-based)
        col_names       = list(df_out.columns)
        ct_dare_col_idx = col_names.index("CtDare") + 1 if "CtDare" in col_names else None
        importo_col_idx = {
            col: col_names.index(col) + 1
            for col in _COLONNE_IMPORTO + _COLONNE_TASSO
            if col in col_names
        }

        # Colori riga per riga
        for pandas_idx in range(len(df_out)):
            excel_row = pandas_idx + 2  # +1 header, +1 base-1

            # Priorità: errore > rosso > verde > azzurro > nessuno
            if pandas_idx in set_errore:
                fill_riga, font_riga = FILL_ERRORE, FONT_ERRORE
            elif pandas_idx in set_rosso:
                fill_riga, font_riga = FILL_ROSSO, FONT_NORMALE
            elif pandas_idx in set_verde:
                fill_riga, font_riga = FILL_VERDE, FONT_VERDE
            elif pandas_idx in set_separatori:
                fill_riga, font_riga = FILL_AZZURRO, FONT_NORMALE
            else:
                fill_riga, font_riga = None, FONT_NORMALE
            ...
            # Formato numerico virgola sulle celle importo/tasso
            for col_name, col_idx in importo_col_idx.items():
                cella = ws.cell(row=excel_row, column=col_idx)
                if cella.value is not None:
                    cella.number_format = FMT_TASSO if col_name in _COLONNE_TASSO else FMT_IMPORTO

            # Cella CtDare gialla (sovrascrive il colore riga)
            if pandas_idx in set_giallo and ct_dare_col_idx:
                cella = ws.cell(row=excel_row, column=ct_dare_col_idx)
                cella.fill = FILL_GIALLO
                cella.font = FONT_GIALLO
...
# Alias retrocompatibilità (chiamato da vecchio app.py senza colori)
def esporta_per_excel(df: pd.DataFrame) -> bytes:
    return esporta_per_excel_colorato(df, [], [], [], [], [])
